                ...
                {isEmpty ? (
                    ...
                    <div className="flex flex-col gap-5">
                        {messages.map((message, idx) => {
                            const text = (message as any).content || (message.parts ? message.parts.map((p: any) => p.text).join('') : '')
                            const isLastBot = message.role === "assistant" && idx === messages.length - 1

                            // Extraer opciones tipo "a)", "A.", "1.", "1)" de la respuesta del bot
                            ...
                            if (message.role === "assistant") {
                                ...
                                if (quickReplies.length >= 2) {
                                    // Remove the options from the displayed text
                                    displayText = text.replace(/^[\s]*(?:[a-dA-D1-4][.)]\s*)(.*)/gm, '').trim();
                                }
                            }
                            ...
                            return (
                                <div key={message.id}>
                                    ...
                                    {isLastBot && !isBusy && quickReplies.length >= 2 && (
